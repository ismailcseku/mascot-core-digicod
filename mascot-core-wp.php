...
/**
 *
Plugin Name: Mascot Core - Digicod
...
Text Domain: mascot-core-digicod
...
/**
 * Main Mascot Core - Digicod Elementor Class
 *
 ...
 * @since 1.0.0
 */
final class Mascot_Core_Digicod_Elementor {
	...
	public function __construct() {
		...
		define( 'MASCOT_CORE_DIGICOD_ASSETS_URI', MASCOT_CORE_DIGICOD_URL_PATH. 'assets' );
	...
	public function i18n() {
		load_plugin_textdomain( 'mascot-core-digicod' );
	}
	...
	public function admin_enque_scripts() {
		wp_enqueue_style( 'mascot-core-custom-admin', MASCOT_CORE_DIGICOD_URL_PATH . 'assets/css/custom-admin.css' );
	}

	public function enque_scripts() {
		if (function_exists('woosw_init')) {
			//wp_enqueue_script('mascot-core-wishlist', MASCOT_CORE_DIGICOD_URL_PATH . 'assets/js/woo/wishlist.js', array('jquery'), MASCOT_CORE_DIGICOD_VERSION, true);
		}
	}
	...
	public function admin_notice_missing_main_plugin() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'mascot-core-digicod' ),
			'<strong>' . esc_html__( 'Mascot Core - Digicod', 'mascot-core-digicod' ) . '</strong>',
			'<strong>' . esc_html__( 'Mascot Core', 'mascot-core-digicod' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_elementor_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'mascot-core-digicod' ),
			'<strong>' . esc_html__( 'Mascot Core - Digicod Elementor', 'mascot-core-digicod' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'mascot-core-digicod' ) . '</strong>',
    ...
    public function add_elementor_custom_icons($settings)
    {
        $settings['flaticon-set-business'] = [
            ...
            'label'         => 'Digicod Business Icons',
            'url'           => '',
            'enqueue'       => array(
                MASCOT_CORE_DIGICOD_URL_PATH . 'assets/flaticon-set-business/style.css',
            ),
            'prefix'        => '',
            'displayPrefix' => '',
            'labelIcon'     => 'flaticon-business-002-graph',
            'ver'           => '1.0',
            'fetchJson'     => MASCOT_CORE_DIGICOD_URL_PATH . 'assets/flaticon-set-business/icon-list.js',
            'native'        => 1,
        ];
				$settings['flaticon-set-digicod'] = [
					'name'          => 'flaticon-set-digicod',
					'label'         => 'Digicod Icon Set',
					'url'           => '',
					'enqueue'       => array(
						MASCOT_CORE_DIGICOD_URL_PATH . 'assets/flaticon-set-digicod/style.css',
					),
					'prefix'        => '',
					'displayPrefix' => '',
	...
	public function admin_notice_minimum_php_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'mascot-core-digicod' ),
			'<strong>' . esc_html__( 'Mascot Core - Digicod Elementor', 'mascot-core-digicod' ) . '</strong>',
			'<strong>' . esc_html__( 'PHP', 'mascot-core-digicod' ) . '</strong>',
...
// Instantiate Mascot_Core_Digicod_Elementor.
new Mascot_Core_Digicod_Elementor();


/* Aqua Resizer
================================================== */
if (!function_exists('mascot_core_digicod_matthewruddy_image_resize')) {
	require_once( plugin_dir_path( __FILE__ ) . 'external-plugins/lib/matthewruddy-image-resizer.php' );
}


if( !function_exists('mascot_core_digicod_theme_installed') ) {
	/**
	* Checks whether theme is installed or not
	* @return bool
	*/
	function mascot_core_digicod_theme_installed() {
		return defined('DIGICOD_FRAMEWORK_VERSION');
	}
}
 ...
 * @return array
 */
if (!function_exists('mascot_core_digicod_get_flat_icons')) :

    function mascot_core_digicod_get_flat_icons()
    {
        ...
        if (empty($data)) {
            ...
            $template_icon_file = MASCOT_CORE_DIGICOD_ABS_PATH . 'assets/flaticon-set-agri/style.css';
            $content = '';
