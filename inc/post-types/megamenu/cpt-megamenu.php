<?php
namespace MASCOTCOREDIGICOD\CPT\MegaMenu;
 ...
 * Singleton class
 * class CPT_MegaMenu
 * @package MASCOTCOREDIGICOD\CPT\MegaMenu;
 */
final class CPT_MegaMenu implements Lib\Mascot_Core_Digicod_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Mega Menu', 'mascot-core-digicod' );
		$this->ptPluralName = esc_html__( 'Parts - Mega Menu', 'mascot-core-digicod' );
		$this->ptKey = 'megamenu';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-megamenu'	=> esc_html__( 'Currently Active Mega Menu', 'mascot-core-digicod' ),
		...
		switch( $columns ) {
			case 'active-megamenu' :
				if( mascot_core_digicod_theme_installed() ) {
					$active_headertop_id = digicod_get_redux_option( 'megamenu-settings-choose-megamenu-cpt-widget-area', 'default' );
