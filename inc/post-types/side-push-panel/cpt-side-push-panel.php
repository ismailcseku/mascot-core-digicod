<?php
namespace MASCOTCOREDIGICOD\CPT\SidePushPanel;
 ...
 * @package MASCOTCOREDIGICOD\CPT\SidePushPanel;
 */
final class CPT_SidePushPanel implements Lib\Mascot_Core_Digicod_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-digicod' );
		$this->ptPluralName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-digicod' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-side-push-panel'	=> esc_html__( 'Currently Active Side Push Panel', 'mascot-core-digicod' ),
		...
		switch( $columns ) {
			case 'active-side-push-panel' :
				if( mascot_core_digicod_theme_installed() ) {
					$active_headertop_id = digicod_get_redux_option( 'header-settings-choose-side-push-panel-cpt-widget-area', 'default' );
