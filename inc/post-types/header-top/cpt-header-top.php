<?php
namespace MASCOTCOREDIGICOD\CPT\HeaderTop;
 ...
 * Singleton class
 * class CPT_HeaderTop
 * @package MASCOTCOREDIGICOD\CPT\HeaderTop;
 */
final class CPT_HeaderTop implements Lib\Mascot_Core_Digicod_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Header Top', 'mascot-core-digicod' );
		$this->ptPluralName = esc_html__( 'Parts - Header Top', 'mascot-core-digicod' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-headertop'	=> esc_html__( 'Currently Active Header Top', 'mascot-core-digicod' ),
		...
		switch( $columns ) {
			case 'active-headertop' :
				if( mascot_core_digicod_theme_installed() ) {
					//default
					$active_headertop_id = digicod_get_redux_option( 'header-settings-choose-header-top-cpt-elementor', 'default' );
					if( $post->ID == $active_headertop_id ) {
						echo '<a class="tm-btn tm-btn-danger tm-btn-sm disabled">Active Header</a>';
					}

					//transparent
					$active_headertop_id = digicod_get_redux_option( 'header-settings-choose-header-top-cpt-elementor-transparent', 'default' );
					if( $post->ID == $active_headertop_id ) {
						echo '<a class="tm-btn tm-btn-danger tm-btn-sm disabled">Active Header Transparent</a>';
					}

					//transparent
					$active_headertop_id = digicod_get_redux_option( 'header-settings-choose-header-top-cpt-elementor-sticky', 'default' );
					if( $post->ID == $active_headertop_id ) {
						echo '<a class="tm-btn tm-btn-danger tm-btn-sm disabled">Active Header Sticky</a>';
					}

					//transparent
					$active_headertop_id = digicod_get_redux_option( 'header-settings-choose-header-top-cpt-elementor-mobile', 'default' );
					if( $post->ID == $active_headertop_id ) {
