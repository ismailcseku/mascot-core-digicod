...
if(!function_exists('mascot_core_digicod_get_maintenance_mode_parts')) {
	/**
	 * Function that Renders Coming Soon Page HTML Codes
	 * @return HTML
	 */
	function mascot_core_digicod_get_maintenance_mode_parts() {
		$params = array();
		...
		if( mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-logo-status' ) ) {
			$params['page_logo'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-logo', false, 'url' );
		}
		$params['page_title'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-title' );
		$params['page_content'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-content' );


		$params['countdown_timer_status'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-countdown-timer-status' );
		$params['enable_social_links'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-enable-social-links' );
...
if (!function_exists('mascot_core_digicod_get_countdown_timer_layout')) {
	/**
	 * Return Countdown Timer Layout HTML
	 */
	function mascot_core_digicod_get_countdown_timer_layout() {
		$params = array();

		$params['launch_date'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-countdown-timer-day' );
		$params['launch_hour'] = mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-countdown-timer-hour' );
...
if (!function_exists('mascot_core_digicod_get_social_links_layout')) {
	/**
	 * Return Social Links HTML
	 */
	function mascot_core_digicod_get_social_links_layout() {
		$params = array();

		//Enabled social links
		$params['social_links'] = mascot_core_digicod_get_redux_option( 'social-links-ordering', false, 'Enabled' );
...
add_filter( 'template_include', 'mascot_core_digicod_maintenance_mode_template', 99 );
function mascot_core_digicod_maintenance_mode_template( $template ) {
	//for maintenance mode
	if( ( ( !current_user_can('edit_themes') || !is_user_logged_in() ) && mascot_core_digicod_get_redux_option( 'maintenance-mode-settings-status' ) ) || mascot_core_digicod_get_url_params( 'view-maintenance-mode' ) ) {
		if( mascot_core_digicod_theme_installed() ) {
			$new_template = MASCOT_CORE_DIGICOD_ABS_PATH . 'inc/maintenance-mode/tpl/page-maintenance-mode-if-theme-installed.php';
		} else {
			$new_template = MASCOT_CORE_DIGICOD_ABS_PATH . 'inc/maintenance-mode/tpl/page-maintenance-mode.php';
		}
