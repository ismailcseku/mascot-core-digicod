...
if ( ! function_exists( 'tm_cursor_mouse_helper_load_scripts_front' ) ) {
	...
	function tm_cursor_mouse_helper_load_scripts_front( $force = false ) {
		...
		$need        = ! $loaded && ( ! $preview_elm ) && (
						$force === true
							|| ( $preview_elm )
							|| (int) mascot_core_digicod_get_redux_option('mouse_helper') > 0
						);
		if ( ! $loaded && $need ) {
			$loaded = true;
...
if ( !function_exists( 'tm_cursor_mouse_helper_localize_script' ) ) {
	...
	function tm_cursor_mouse_helper_localize_script($vars) {
		$vars['mouse_helper']            = (int) mascot_core_digicod_get_redux_option('mouse_helper');
		$vars['mouse_helper_delay']      = max( 1, min( 20, (int) mascot_core_digicod_get_redux_option('mouse_helper_delay') ) );
		$vars['mouse_helper_centered']   = (int) mascot_core_digicod_get_redux_option('mouse_helper_centered');
		$vars['msg_mouse_helper_anchor'] = (int) mascot_core_digicod_get_redux_option('mouse_helper') > 0 ? esc_attr__( 'Scroll to', 'mascot-core-digicod' ) : '';
		return $vars;
	}
}
...
if ( ! function_exists( 'tm_cursor_mouse_helper_add_redux_options' ) ) {
	...
	function tm_cursor_mouse_helper_add_redux_options() {
		...
		$opt_name = "digicod_redux_theme_opt";

		// -> START Custom HTML/JS Codes
		Redux::setSection( $opt_name, array(
			'title'  => esc_html__( 'Cursor Mouse helper', 'mascot-core-digicod' ),
			...
			'fields' => array(
				array(
					'id'       => 'mouse_helper',
					'type'     => 'switch',
					'title'    => esc_html__( 'Show mouse helper', 'mascot-core-digicod' ),
					'subtitle' => esc_html__( 'Display animated helper near the mouse cursor on desktop and notebooks', 'mascot-core-digicod' ),
					'default'  => 0,
				...
				array(
					...
					'type'     => 'switch',
					'title'    => esc_html__( 'Always visible', 'mascot-core-digicod' ),
					'subtitle' => esc_html__( 'Display the mouse helper permanently or only when hovering over the corresponding object', 'mascot-core-digicod' ),
					'default'  => 0,
					'on'       => esc_html__( 'Yes', 'mascot-core-digicod' ),
					'off'      => esc_html__( 'No', 'mascot-core-digicod' ),
					'required' => array(
				...
				array(
					...
					'type'     => 'switch',
					'title'    => esc_html__( 'Centered', 'mascot-core-digicod' ),
					'subtitle' => esc_html__( 'Place the center of the helper in the cursor position', 'mascot-core-digicod' ),
					'default'  => 0,
					'on'       => esc_html__( 'Yes', 'mascot-core-digicod' ),
					'off'      => esc_html__( 'No', 'mascot-core-digicod' ),
					'required' => array(
				...
				array(
					...
					'type'          => 'slider',
					'title'         => esc_html__( 'Delay', 'mascot-core-digicod' ),
					'subtitle'      => esc_html__( 'The coefficient of lag between the helper and the cursor (1 - the helper moves with the cursor)', 'mascot-core-digicod' ),
					'desc'          => '',
					'default'       => 10,
					'min'           => 1,
					'step'          => 1,
				...
				array(
					...
					'type'     => 'button_set',
					'compiler' =>true,
					'title'    => esc_html__( 'System cursor', 'mascot-core-digicod' ),
					'subtitle' => esc_html__( 'Replace system cursor with custom image', 'mascot-core-digicod' ),
					'options' => array(
						"0"    => esc_html__( 'Default', 'mascot-core-digicod' ),
						"1"    => esc_html__( 'Replace', 'mascot-core-digicod' ),
						"hide" => esc_html__( 'Hide', 'mascot-core-digicod' ),
					),
					'default' => '0',
				),
				array(
					...
					'type'     => 'media',
					'url'      => false,
					'title'    => esc_html__( 'Default cursor image', 'mascot-core-digicod' ),
				...
				array(
					...
					'type'     => 'media',
					'url'      => false,
					'title'    => esc_html__( 'Default cursor image (alternative)', 'mascot-core-digicod' ),
				...
				array(
					...
					'type'     => 'media',
					'url'      => false,
					'title'    => esc_html__( 'Cursor image over links', 'mascot-core-digicod' ),
				...
				array(
					...
					'type'     => 'media',
					'url'      => false,
					'title'    => esc_html__( 'Cursor image over links (alternative)', 'mascot-core-digicod' ),
					'subtitle' => esc_html__( 'Select or upload image to use it as alternative cursor', 'mascot-core-digicod' ),
					'compiler' => 'true',
					'desc'     => '',
					'required' => array(
...
if ( !function_exists( 'tm_cursor_mouse_helper_add_to_html' ) ) {
	...
	function tm_cursor_mouse_helper_add_to_html() {
		if ( (int) mascot_core_digicod_get_redux_option( 'mouse_helper' ) > 0 ) {
			// Add mouse helper layout
			?><div class="<?php
				echo esc_attr( apply_filters( 'tm_addons_filter_mouse_helper_classes',
							'tm_cursor_mouse_helper'
							. ( (int) mascot_core_digicod_get_redux_option( 'mouse_helper_permanent' ) > 0
									? ' tm_cursor_mouse_helper_permanent'
									: ''
									)
							. ( (int) mascot_core_digicod_get_redux_option( 'mouse_helper_centered' ) > 0
...
if ( !function_exists( 'tm_cursor_mouse_helper_replace_system_cursor' ) ) {
	...
	function tm_cursor_mouse_helper_replace_system_cursor( $classes ) {
		if ( (int) mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor' ) == 1 ) {
			$classes[]  = 'tm_addons_custom_cursor';
			$cur_defa   = mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor_default', false, 'url' );
			$cur_defa2  = mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor_default2', false, 'url' );
			$cur_links  = mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor_links', false, 'url' );
			$cur_links2 = mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor_links2', false, 'url' );
			if ( ! empty( $cur_defa ) ) {
		...
		} else if ( in_array( mascot_core_digicod_get_redux_option( 'mouse_helper_replace_cursor' ), array( '2', 'hide' ) ) ) {
					...
					$params = array_merge(
								...
								array(
									array(
										...
										'label' => __("Highlight on mouse hover", 'mascot-core-digicod'),
										'description' => wp_kses_data( __( 'Used only if option "Mouse helper" is on in the Theme Panel - ThemeREX Addons settings', 'mascot-core-digicod' ) ),
					...
					$params = array_merge(
						...
						array(
							array(
								...
								"heading" => esc_html__("Highlight on mouse hover", 'mascot-core-digicod'),
								...
								"std" => "0",
								"value" => array(esc_html__("Highlight title", 'mascot-core-digicod') => "1" ),
								"type" => "checkbox"
							),
						),
						array_slice( $params, $k, null, true)
...
if ( !function_exists( 'tm_cursor_mouse_helper_highlight_add_title_class_tpl' ) ) {
	...
	function tm_cursor_mouse_helper_highlight_add_title_class_tpl( $class, $sc, $args=array() ) {
		if ( ! empty($args['title_color']) ) {
			if ( ! empty($args['mouse_helper_highlight']) && (int) mascot_core_digicod_get_redux_option('mouse_helper') > 0 ) {
...
if ( ! function_exists( 'tm_cursor_mouse_helper_highlight_add_heading_param_in_elementor' ) ) {
	...
	function tm_cursor_mouse_helper_highlight_add_heading_param_in_elementor( $element, $section_id, $args ) {
		...
		if ( 'heading' == $el_name && 'section_title' === $section_id && (int) mascot_core_digicod_get_redux_option('mouse_helper') > 0 ) {
...
if ( ! function_exists( 'tm_cursor_mouse_helper_highlight_before_render_heading_in_elementor' ) ) {
	...
	function tm_cursor_mouse_helper_highlight_before_render_heading_in_elementor( $element ) {
		if ( is_object( $element ) && (int) mascot_core_digicod_get_redux_option( 'mouse_helper' ) > 0 ) {
			$el_name = $element->get_name();
			if ( 'heading' == $el_name ) {
	...
	function tm_cursor_mouse_helper_add_params_to_elements($element, $section_id, $args) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'common' ) ) && $section_id == '_section_responsive' && (int) mascot_core_digicod_get_redux_option( 'mouse_helper' ) > 0 ) {
			...
			$element->add_control( 'mouse_helper', array(
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label' => __("Enable mouse helper", 'mascot-core-digicod'),
				'label_on' => __( 'On', 'mascot-core-digicod' ),
				'label_off' => __( 'Off', 'mascot-core-digicod' ),
				'return_value' => '1',
				'default' => '',
			...
			$element->add_control( 'mouse_helper_action', array(
				...
				'label' => __( 'Action', 'mascot-core-digicod' ),
				'label_block' => false,
				'options' => apply_filters( 'tm_addons_filter_mouse_helper_action', array(
					'hover' => esc_html__( 'Hover', 'mascot-core-digicod' ),
				) ),
				'condition' => array(
					'mouse_helper' => '1'
				),
			...
			if ( mascot_core_digicod_get_redux_option('mouse_helper_replace_cursor') != 'hide' ) {
				$element->add_control( 'mouse_helper_hide_cursor', array(
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'label' => __("Hide system cursor", 'mascot-core-digicod'),
					'label_on' => __( 'On', 'mascot-core-digicod' ),
					'label_off' => __( 'Off', 'mascot-core-digicod' ),
					'return_value' => '1',
					'default' => '',
			...
			if ( (int) mascot_core_digicod_get_redux_option('mouse_helper_centered') == 0 ) {
				$element->add_control( 'mouse_helper_centered', array(
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'label' => __("Cursor in the center", 'mascot-core-digicod'),
					'label_on' => __( 'On', 'mascot-core-digicod' ),
					'label_off' => __( 'Off', 'mascot-core-digicod' ),
					'return_value' => '1',
					'default' => '',
			...
			$element->add_control( 'mouse_helper_magnet', array(
				'label' => __( 'Magnet distance', 'mascot-core-digicod' ),
			...
			$element->add_control( 'mouse_helper_bg_color', array(
				'label' => __( 'Background color', 'mascot-core-digicod' ),
				'label_block' => false,
...
//				),
			...
			$element->add_control( 'mouse_helper_bd_color', array(
				'label' => __( 'Border color', 'mascot-core-digicod' ),
				'label_block' => false,
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
//				'global' => array(
...
//				),
			...
			$element->add_control( 'mouse_helper_color', array(
				'label' => __( 'Text color', 'mascot-core-digicod' ),
				'label_block' => false,
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '#fff',
//				'global' => array(
...
//				),
			...
			$element->add_control( 'mouse_helper_mode', array(
				'type' => \Elementor\Controls_Manager::SELECT,
				'label' => __( 'Overlay mode', 'mascot-core-digicod' ),
				'label_block' => false,
				'options' => apply_filters( 'tm_addons_filter_mouse_helper_mode', array(
					'' => esc_html__( 'Default', 'mascot-core-digicod' ),
					'normal' => esc_html__( 'Normal', 'mascot-core-digicod' ),
					'multiply'  => esc_html__( 'Multiply', 'mascot-core-digicod' ),
					'screen'  => esc_html__( 'Screen', 'mascot-core-digicod' ),
					'overlay'  => esc_html__( 'Overlay', 'mascot-core-digicod' ),
					'darken'  => esc_html__( 'Darken', 'mascot-core-digicod' ),
					'lighten'  => esc_html__( 'Lighten', 'mascot-core-digicod' ),
					'color-dodge'  => esc_html__( 'Color Dodge', 'mascot-core-digicod' ),
					'color-burn'  => esc_html__( 'Color Burn', 'mascot-core-digicod' ),
					'hard-light'  => esc_html__( 'Hard Light', 'mascot-core-digicod' ),
					'soft-light'  => esc_html__( 'Soft Light', 'mascot-core-digicod' ),
					'difference'  => esc_html__( 'Difference', 'mascot-core-digicod' ),
					'exclusion'  => esc_html__( 'Exclusion', 'mascot-core-digicod' ),
					'hue'  => esc_html__( 'Hue', 'mascot-core-digicod' ),
					'saturation'  => esc_html__( 'Saturation', 'mascot-core-digicod' ),
					'color'  => esc_html__( 'Color', 'mascot-core-digicod' ),
					'luminosity'  => esc_html__( 'Luminosity', 'mascot-core-digicod' ),
				) ),
				'condition' => array(
					'mouse_helper' => '1'
				),
			...
			$element->add_control( 'mouse_helper_axis', array(
				'type' => \Elementor\Controls_Manager::SELECT,
				'label' => __( 'Motion axis', 'mascot-core-digicod' ),
				'label_block' => false,
				'options' => array(
					'xy' => esc_html__( 'Both', 'mascot-core-digicod' ),
					'x'  => esc_html__( 'X only', 'mascot-core-digicod' ),
					'y'  => esc_html__( 'Y only', 'mascot-core-digicod' ),
				),
				'condition' => array(
					'mouse_helper' => '1'
				),
			...
			$element->add_control( 'mouse_helper_delay', array(
				'label' => __( 'Motion delay', 'mascot-core-digicod' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => array(
					'size' => (int) mascot_core_digicod_get_redux_option( 'mouse_helper_delay' ),
					'unit' => 'px'
				),
				'range' => array(
					'px' => array(
			...
			$element->add_control( 'mouse_helper_text', array(
				...
				'label' => __( 'Helper text', 'mascot-core-digicod' ),
				'label_block' => false,
				'condition' => array(
					'mouse_helper' => '1'
				),
			...
			$element->add_control( 'mouse_helper_text_size', array(
				'label' => __( 'Text size', 'mascot-core-digicod' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => array(
					'size' => '',
					'unit' => 'px'
			...
			$element->add_control( 'mouse_helper_text_round', array(
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label' => __("Rotate text in a circle", 'mascot-core-digicod'),
				'label_on' => __( 'On', 'mascot-core-digicod' ),
				'label_off' => __( 'Off', 'mascot-core-digicod' ),
				'return_value' => '1',
				'default' => '',
			...
			$element->add_control( 'mouse_helper_icon_size', array(
				'label' => __( 'Icon size', 'mascot-core-digicod' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => array(
					'size' => '',
					'unit' => 'px'
			...
			$element->add_control( 'mouse_helper_icon_color', array(
				'label' => __( 'Icon color', 'mascot-core-digicod' ),
				'label_block' => false,
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
//				'global' => array(
...
//				),
			...
			$element->add_control( 'mouse_helper_image', array(
				...
				'label' => __( 'Image', 'mascot-core-digicod' ),
				'default' => array(
					'url' => '',
				),
				'condition' => array(
			...
			$element->add_control( 'mouse_helper_layout', array(
				...
				'label' => __( 'Custom layout', 'mascot-core-digicod' ),
				'condition' => array(
					'mouse_helper' => '1'
				),
			) );
			...
			$element->add_control( 'mouse_helper_callback', array(
				'type' => \Elementor\Controls_Manager::TEXT,
				'label' => __( 'JS Callback', 'mascot-core-digicod' ),
				'condition' => array(
					'mouse_helper' => '1'
				),
			) );
		...
		if ( is_object($element) ) {
			...
			if ( ! empty( $mouse_helper ) ) {
				...
				$element->add_render_attribute( '_wrapper', array(
					...
					'data-mouse-helper-centered' => (int) mascot_core_digicod_get_redux_option('mouse_helper_centered') == 0
					...
					'data-mouse-helper-delay' => ! empty( $settings['mouse_helper_delay'] )
													...
													: ( mascot_core_digicod_get_redux_option( 'mouse_helper_delay' )
														? (int) mascot_core_digicod_get_redux_option( 'mouse_helper_delay' )
														: 10
														),
